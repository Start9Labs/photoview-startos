        ...
        outer: for(const key in a){
            ...
            for (const [keyParser, valueParser] of parsers){
                ...
                if ("error" in valueState) {
                    const { error: error1  } = valueState;
                    error1.keys.push("" + newKey);
                    parseError.unshift(error1);
                    continue;
                }
                delete answer[key];
...
class ShapeParser {
    ...
    parse(a, onParse) {
        ...
        for(const key in parserMap){
            if (key in value) {
                const parser1 = parserMap[key];
                const state = parser1.enumParsed(a[key]);
                if ("error" in state) {
                    const { error  } = state;
...
const migration = mod3.migrations.fromMapping({}, "2.3.13");
